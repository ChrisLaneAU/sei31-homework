...
const planTrip = (lineStart, stationStart, lineEnd, stationEnd) => displayTrip(
  ...
  lineEnd !== lineStart ? stops(lineEnd, getStationIndex(lineEnd, "Union Square"), getStationIndex(lineEnd, stationEnd)) : [] // lastStops
);
*/
...
const stops = (line, startIndex, endIndex) => { const orderedStops = isReversed(startIndex, endIndex) ? lines[line].stationsReversed() : lines[line].stations; return orderedStops.filter((station, index) => {if (isReversed(startIndex, endIndex))return station && index > endIndex && index <= startIndex;return station && index > startIndex && index <= endIndex;});
};const planTrip = (lineStart, stationStart, lineEnd, stationEnd) => displayTrip( stops(lineStart, getStationIndex(lineStart, stationStart), getStationIndex(lineStart, "Union Square")).length + (stops(lineEnd, getStationIndex(lineEnd, stationEnd), getStationIndex(lineEnd, "Union Square")).length || 0),  lineStart, stops(lineStart, getStationIndex(lineStart, stationStart), getStationIndex(lineStart, "Union Square")), lineEnd !== lineStart ? stops(lineEnd, getStationIndex(lineEnd, "Union Square"), getStationIndex(lineEnd, stationEnd)) : []);
